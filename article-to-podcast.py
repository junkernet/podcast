        ...
        try:
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                if isinstance(data, dict) and data.get("results"):
                    r = data["results"][0]
                    tavily_text = r.get("raw_content", r.get("text", ""))
...
def clean_for_tts(text: str) -> str:
    """Clean text for TTS: strip URLs, emails, boilerplate."""
    # Strip URLs
    text = re.sub(r"https?://\S+", "", text)
    # Strip email addresses
    text = re.sub(r"\S+@\S+\.\S+", "", text)
    # Remove "Share this article" / social boilerplate
    for phrase in [
...
def text_to_mp3(text: str, output_path: Path) -> None:
    ...
    try:
        for i, chunk in enumerate(chunks):
            ...
            response = client.audio.speech.create(
                model=TTS_MODEL, voice=TTS_VOICE, input=chunk
            )
...
def generate_intro_audio(intro_text: str, main_mp3: Path) -> None:
    ...
    try:
        ...
        # Read existing main audio
        main_audio = main_mp3.read_bytes()
        intro_audio = Path(intro_tmp.name).read_bytes()
...
def generate_webvtt(mp3_path: Path) -> Path:
    """Transcribe MP3 via Whisper and generate WebVTT."""
    client = OpenAI()
...
<rss version="2.0"
  ...
  <channel>
    ...
    <link>{BASE_URL}</link>
    <description>Personal audio feed.</description>
...
def add_episode_to_feed(
    ...
    item_lines = [
        ...
        f"      <itunes:summary>{_xml_escape(description[:255])}</itunes:summary>",
    ]
    ...
    FEED_FILE.write_text(feed_text)
...
def git_commit_and_push(title: str):
...
def list_episodes():
    ...
    if not items:
        ...
        return
    print(f"📻 Private Feed — {len(items)} episode(s)\n")
    for item in items:
...
def main():
    ...
    pushed = git_commit_and_push(episode_title)
